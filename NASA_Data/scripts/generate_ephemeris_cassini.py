...
def _selftest_regex():
    ...
    sample = 'No ephemeris for target "Cassini (spacecraft)" prior to A.D. 1997-OCT-15 08:43:00.0000 UT'
...
def main():
    ...
    out_root = repo_root / "NASA_Data" / "events_in_our_solar_system" / "output" / "cassini"
    ...
    # Cassini (daily)
    t_cassini, pv_cassini, sig_cassini = horizons_vectors_chunked(CASSINI_ID, LAUNCH_TIME, stop_time, "1 d")
    cassini_json = {
        "schema": "mcs-ephem-v1",
        "t_jd": t_cassini,
        "pv": pv_cassini,
        "meta": {
            ...
            "object": {"name": "Cassini", "spkid": CASSINI_ID},
            "signature": sig_cassini,
        },
    }
    write_json(ephem_dir / "cassini_1d.json", cassini_json)

    # Higher-resolution windows for key milestones
    windows = [
        (
            "cassini_jupiter_flyby_10m",
            "2013-10-09 12:00:00",
            "2013-10-10 06:00:00",
            "10 m",
            "cassini_jupiter_flyby_10m.json",
        ),
        (
            "cassini_saturn_orbit_insertion_10m",
            "2016-07-04 18:00:00",
            "2016-07-05 12:00:00",
            "10 m",
            "cassini_saturn_orbit_insertion_10m.json",
        ),
    ]

    for ds_id, s, e, step, fn in windows:
        t_hi, pv_hi, sig_hi = horizons_vectors_chunked(CASSINI_ID, s, e, step)
        hi_json = {
            "schema": "mcs-ephem-v1",
            "t_jd": t_hi,
            "pv": pv_hi,
    ...
    manifest = {
        ...
        "datasets": [
            ...
            {"id": "cassini_1d", "file": "ephemeris/cassini_1d.json"},
            {"id": "cassini_jupiter_flyby_10m", "file": "ephemeris/cassini_jupiter_flyby_10m.json"},
            {"id": "cassini_saturn_orbit_insertion_10m", "file": "ephemeris/cassini_saturn_orbit_insertion_10m.json"},
        ],
    }
